...
// - To use 16-bit indices + allow large meshBuckets: backend need to set 'io.BackendFlags |= ImGuiBackendFlags_RendererHasVtxOffset' and handle ImDrawCmd::VtxOffset (recommended).
...
namespace ImGui
{
    ...
    // - Speed are per-pixel of mouse movement (v_speed=0.2f: mouse needs to move by 5 pixels to increase value by 1). For gamepad/keyboard navigation, minimum movementSpeed is Max(v_speed, minimum_step_at_given_precision).
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_RendererHasVtxOffset  = 1 << 3,   // Backend Renderer supports ImDrawCmd::VtxOffset. This enables output of large meshBuckets (64K+ vertices) while still using 16-bit indices.
...
//   this fields allow us to render meshBuckets larger than 64K vertices while keeping 16-bit indices.
...
struct ImDrawCmd
{
    ...
    unsigned int    VtxOffset;          // 4    // Start offset in vertex buffer. ImGuiBackendFlags_RendererHasVtxOffset: always 0, otherwise may be >0 to support meshBuckets larger than 64K vertices with 16-bit indices.
...
enum ImDrawListFlags_
{
    ...
    ImDrawListFlags_AllowVtxOffset          = 1 << 3,  // Can emit 'VtxOffset > 0' to allow large meshBuckets. Set when 'ImGuiBackendFlags_RendererHasVtxOffset' is enabled.
};
